...
export const signUpWithEmail = async (req, res, next) => {
  try {
    ...
    const isExisitsUser = await User.findOne({ where: { email } });
    if (isExisitsUser) {
      throw new Error('User already exists.');
    }
    const hashedPassword = await hash(password, 10);
    const newUser = await User.create({ name, email, password: hashedPassword });
    const accessToken = sign({ name, id: newUser.id }, process.env.JWT_SECRET, {
      expiresIn: process.env.ACCESS_TOKEN_EXPIRES_IN,
    });
    const refreshToken = sign({ name, id: newUser.id }, process.env.JWT_SECRET, {
...
export const signInWithEmail = async (req, res, next) => {
  try {
    ...
    if (isUserExists && compareSync(password, isUserExists.password)) {
      accessToken = sign(
        { name: isUserExists.name, id: isUserExists.id },
...
export const signUpWithGoogle = async (req, res, next) => {
  ...
  if (!code) {
    res.status(HTTP_STATUS.BAD_REQUEST).json({
      success: false,
      message: RESPONSE_MESSAGES.USERS.AUTH_CODE_NOT_FOUND,
    });
  }
...
export const signInWithGoogle = async (req, res, next) => {
  ...
  try {
    ...
    const payload = { name, id: isUserExists.id };
    const accessToken = sign(payload, process.env.JWT_SECRET, {
      expiresIn: process.env.ACCESS_TOKEN_EXPIRES_IN,
    });
    const refreshToken = sign(payload, process.env.JWT_SECRET, {
