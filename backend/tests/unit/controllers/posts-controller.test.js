...
// Also mock sequelize helpers used in the controller
jest.mock('sequelize', () => ({
  fn: jest.fn(),
  col: jest.fn(),
  where: jest.fn(),
...
describe('createPostHandler', () => {
  it('Post creation: Success - All fields are valid', async () => {
    ...
    Post.create = jest.fn().mockResolvedValueOnce(postObject);
  ...
  it('Post creation: Failure - Internal server error', async () => {
    ...
    Post.create = jest.fn().mockRejectedValueOnce(new Error(RESPONSE_MESSAGES.COMMON.INTERNAL_SERVER_ERROR));
...
describe('getAllPostsHandler', () => {
  ...
  it('Get all posts: Failure - Internal Server Error', async () => {
    ...
    Post.findAll = jest.fn().mockRejectedValueOnce(new Error(RESPONSE_MESSAGES.COMMON.INTERNAL_SERVER_ERROR));
...
describe('getFeaturedPostsHandler', () => {
  it('Get featured posts: Success - Retrieving all featured posts list', async () => {
    ...
    Post.findAll = jest.fn().mockResolvedValueOnce(mockFeaturedPosts);
...
describe('getPostByIdHandler', () => {
  it('Get post by ID: Success - Retrieving Specific Post', async () => {
    const req = createRequestObject({ params: { id: 1 } });

    const mockPost = createPostObject({ id: 1 });
  ...
  it('Get post by ID: Failure - Post not found (Specified post ID is invalid)', async () => {
    const req = createRequestObject({ params: { id: 999999 } });
  ...
  it('Get post by ID: Failure - Internal Server Error', async () => {
    ...
    Post.findByPk = jest.fn().mockRejectedValueOnce(new Error(RESPONSE_MESSAGES.COMMON.INTERNAL_SERVER_ERROR));
...
describe('updatePostHandler', () => {
  it('Update post: Success - Modifying post content', async () => {
    ...
    const mockPost = createPostObject({ id: 1, title: 'Updated Post' });
    const mockInstance = {
      ...mockPost,
      update: jest.fn().mockResolvedValueOnce(mockPost),
    };

    Post.findByPk = jest.fn().mockResolvedValueOnce(mockInstance);
